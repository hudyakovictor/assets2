...
import { IconCheck, IconLightning, IconFlame } from "./Icons";
        ...
        {phase === "feedback" && (
          <>
            <div className="in d2 box glass-good" style={{ display: "flex", gap: 10, alignItems: "center" }}>
              <span className="pop okc p-1"><IconCheck className="w-4 h-4" color="#04222a" /></span>
      ...
      <div className="gs-body">
        ...
        {SCENARIOS.map((s, i) => (
          <button key={s.name} onClick={tap(() => setPick(i), "select")} className={`in d${i + 1} box scenario-row`} style={{ boxShadow: pick === i ? "inset 0 0 0 2px var(--acc), 0 0 22px rgba(58,217,230,.25)" : undefined }}>
            ...
            <div className="chip" style={{ minHeight: 26, padding: "4px 8px", display: "inline-flex", alignItems: "center", gap: 4 }}>
    ...
    body = (
      <div className="gs-body">
        <div className="in" style={{ display: "flex", gap: 12, alignItems: "center" }}>
          ...
          <div>
            <div style={{ fontWeight: 700, fontSize: 17 }}>Игрок</div>
            <div className="t s" style={{ display: "flex", alignItems: "center", gap: 6 }}>
              <span>LVL {page.topBarState.lvl}</span>
              <span>•</span>
              <span style={{ display: "inline-flex", alignItems: "center", gap: 3, color: "var(--warm)" }}>
                <IconFlame className="w-3.5 h-3.5" />
                <b>Серия 4</b>
              </span>
              <span>•</span>
              <span>Точность 68%</span>
            </div>
          </div>
        </div>
